...
Source4: %{name}-create_barnyard2_mysql.sql
...
%{__install} -pm 644 %SOURCE4 %buildroot/%_datadir/%name/sql/%name-create_barnyard2_mysql.sql
...
BuildRequires: php5
Requires: php5
Requires: adodb5 => 5.10
Requires: libmysqlclient18
Requires: barnyard2 >= 2.12
Requires: %{name}
...
echo " BASE web SAMPLE htpasswd file located at /srv/www/.htpasswd"
echo " BASE web passwd for user snortadmin set to : snortadmin "
echo ""
echo " please restart apache due new installed /etc/apache2/conf.d/base.conf"

test -f /srv/www/.htpasswd || echo "snortadmin:$apr1$FI5DlqcZ$8L.z/aIeAf9j1UFr4Y/GW/" >/srv/www/.htpasswd
...
%changelog
* Sun Mar 23 2014 support@remsnet.de -r2
- fixed typo in /etc/apache2/conf.d/base.conf"
- default htpasswd moved to /srv/www/.htpasswd
* Sun Mar 23 2014 support@remsnet.de -r2
